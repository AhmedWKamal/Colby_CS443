...
def dist_dep_net(I, A, B, C, e_sigma, i_sigma, kerSz, t_max, dt):
    ...
    TODO:
    - Create two small 1D Gaussian kernels (shape of each =(kerSz,)) with different sigma values
...
def dist_dep_net_image(I, A, i_sigma, kerSz, t_max, dt):
    ...
    you can downscale your images or solve for and replace the ODE with the steady state solution.
    ...
    - You may generate a 2D Gaussian kernels in any manner that you like. You can assume that
    the covariance matrix is symmetric (i.e. Gaussian is a circularly symmetric blob).
        - One way is to generate a 1D one like before then use the matrix multiplication "trick"
        (outer product) from the end of Project 0 to make a symmetric 2D Gaussian kernel
        (a 1x25 1D kernel should make a 25x25 2D kernel).
